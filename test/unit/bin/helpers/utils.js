...
const testgrid = require('testgrid-local');
...
describe('utils', () => {
    ...
    it(`should return value depending on validation messages`, () => {
      expect(
        utils.getErrorCodeFromMsg(
          constant.validationMessages.EMPTY_TESTGRID_JSON
        )
      ...
      expect(
        utils.getErrorCodeFromMsg(
          'Please use --config-file <path to testgrid.json>.'
        )
      ...
      expect(
        utils.getErrorCodeFromMsg('Invalid testgrid.json file.')
  ...
  describe('isParallelValid', () => {
    ...
    it('should return false for a string which is not a number', () => {
      ...
      expect(utils.isParallelValid('testgrid')).to.be.equal(false);
    });
  ...
  describe('validateBstackJson', () => {
    it('should reject with SyntaxError for empty file', () => {
      ...
      return utils.validateBstackJson(bsConfigPath).catch((error) => {
        sinon.match(error, 'Invalid testgrid.json file');
      });
    });
  ...
  describe('setTestEnvs', () => {
    ...
    it('set env only from testgrid.json env param', () => {
      let bsConfig = {
        run_settings: {
          env: {
            env1: 'value1',
    ...
    it('merges env from args and testgrid.json env param', () => {
      let argsEnv = 'env3=value3, env4=value4';
      let bsConfig = {
        run_settings: {
          env: {
    ...
    it('merges env from args and testgrid.json env param but give preceedence to args', () => {
  ...
  describe('setLocal', () => {
    afterEach(function () {
      sinon.restore();
      delete process.env.TESTGRID_LOCAL;
    });
    ...
    it('should not change local in bsConfig if process.env.TESTGRID_LOCAL is undefined', () => {
      let bsConfig = {
        connection_settings: {
          local: true,
        },
    ...
    it('should change local to true in bsConfig if process.env.TESTGRID_LOCAL is set to true', () => {
      let bsConfig = {
        connection_settings: {
          local: false,
        },
      ...
      let args = {};
      process.env.TESTGRID_LOCAL = true;
      utils.setLocal(bsConfig, args);
      expect(bsConfig.connection_settings.local).to.be.eq(true);
    });

    it('should change local to false in bsConfig if process.env.TESTGRID_LOCAL is set to false', () => {
      let bsConfig = {
        connection_settings: {
          local: true,
        },
      ...
      let args = {};
      process.env.TESTGRID_LOCAL = false;
      utils.setLocal(bsConfig, args);
  ...
  describe('setupLocalTesting', () => {
    ...
    it('if bsconfig local is true then promise should return a testgrid local object', () => {
      let bsConfig = {
        auth: {
          access_key: 'xyz',
        },
  ...
  describe('setLocalIdentifier', () => {
    beforeEach(function () {
      delete process.env.TESTGRID_LOCAL_IDENTIFIER;
    });

    afterEach(function () {
      delete process.env.TESTGRID_LOCAL_IDENTIFIER;
    });
    it('should not generate local_identifier if args.localIdentifier & process.env.TESTGRID_LOCAL_IDENTIFIER is undefined', () => {
      let bsConfig = {
        connection_settings: {
          local: true,
        },
    ...
    it('should change local identifier to local_identifier in bsConfig if process.env.TESTGRID_LOCAL_IDENTIFIER is set to local_identifier', () => {
      let bsConfig = {
        connection_settings: {
      ...
      let args = {};
      process.env.TESTGRID_LOCAL_IDENTIFIER = 'local_identifier';
      utils.setLocalIdentifier(bsConfig, args);
    ...
    it('should set local identifier in connection_settings in bsConfig if process.env.TESTGRID_LOCAL_IDENTIFIER is present & not set in bsConfig', () => {
      let bsConfig = {
        connection_settings: {},
      };
      let args = {};
  ...
  describe('setUsername', () => {
    beforeEach(function () {
      delete process.env.TESTGRID_USERNAME;
    });
    ...
    it('should set username if process.env.TESTGRID_USERNAME is present and args.username is not present', () => {
      let bsConfig = {
        auth: {
          username: 'test',
        },
      };
      process.env.TESTGRID_USERNAME = 'username';
      utils.setUsername(bsConfig, {});
      expect(bsConfig.auth.username).to.be.eq('username');
    });

    it('should set username to default if process.env.TESTGRID_USERNAME and args.username is not present', () => {
      let bsConfig = {
        auth: {
          username: 'test',
        },
  ...
  describe('setAccessKey', () => {
    beforeEach(function () {
      delete process.env.TESTGRID_ACCESS_KEY;
    });
    ...
    it('should set access_key if process.env.TESTGRID_ACCESS_KEY is present and args.access_key is not present', () => {
      let bsConfig = {
        auth: {
          access_key: 'test',
        },
      };
      process.env.TESTGRID_ACCESS_KEY = 'access_key';
      utils.setAccessKey(bsConfig, {});
      expect(bsConfig.auth.access_key).to.be.eq('access_key');
    });

    it('should set access_key to default if process.env.TESTGRID_ACCESS_KEY and args.access_key is not present', () => {
      let bsConfig = {
        auth: {
          access_key: 'test',
        },
  ...
  describe('setDefaults', () => {
    ...
    it('should set setDefaults if process.env.TESTGRID_USERNAME is present and args.username is not present', () => {
      let bsConfig = { run_settings: {} };
      process.env.TESTGRID_USERNAME = 'username';
    ...
    it('should not set setDefaults if process.env.TESTGRID_USERNAME and args.username is not present', () => {
      let bsConfig = { run_settings: {} };
      utils.setDefaults(bsConfig, {});
  ...
  describe('stopTestGridBuild', () => {
    ...
    it('message thrown if API deprecated', async () => {
      ...
      await utils.stopTestGridBuild(bsConfig, args, buildId, rawArgs);
  ...
  describe('setProcessHooks', () => {
    it('should handle "SIGINT" event', (done) => {
      ...
      let stopTestGridBuildStub= sinon.stub(utils, 'stopTestGridBuild').returns(Promise.resolve(true));
      ...
      process.on('SIGINT', () => {
        ...
        sinon.assert.calledOnce(stopTestGridBuildStub);
        done();
      });
      process.emit('SIGINT');
      sinon.stub.restore();
