...
exports.createAccessibilityTestRun = async (user_config, framework) => {
  ...
  try {
    if (!this.isAccessibilitySupportedCypressVersion(user_config.run_settings.cypress_config_file) ){
      ...
      process.env.TESTGRID_TEST_ACCESSIBILITY = 'false';
    ...
    const data = {
      ...
      'testgridAutomation': process.env.TESTGRID_AUTOMATION === 'true'
    };
    ...
    if (process.env.BS_A11Y_JWT) {
      process.env.TESTGRID_TEST_ACCESSIBILITY = 'true';
    }
    logger.debug(`TestGrid Accessibility Automation Test Run ID: ${response.data.data.id}`);
   
    this.setAccessibilityCypressCapabilities(user_config, response.data);
    setAccessibilityEventListeners();
    helper.setTestgridCypressCliDependency(user_config);

  } catch (error) {
    if (error.response) {
      logger.error(
        `Exception while creating test run for TestGrid Accessibility Automation: ${
...
const getAccessibilityCypressCommandEventListener = () => {
  return (
    `require('testgrid-cypress-cli/bin/accessibility-automation/cypress');`
  );
}
