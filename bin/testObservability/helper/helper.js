...
exports.debug = (text, shouldReport = false, throwable = null) => {
  if (process.env.TESTGRID_OBSERVABILITY_DEBUG === "true" || process.env.TESTGRID_OBSERVABILITY_DEBUG === "1") {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    request(options, function callback(error, response, body) {
      if(error) {
        ...
        reject(response && response.body ? response.body : `Received response from TestGrid Server with status : ${response.statusCode}`);
      } else {
        try {
          if(typeof(body) !== 'object') body = JSON.parse(body);
        } catch(e) {
          if(!url.includes('/stop')) {
            reject('Not a JSON response from TestGrid Server');
          }
        }
        resolve({
          data: body
...
exports.getPackageVersion = (package_, bsConfig = null) => {
  ...
  /* Read package version from npm_dependencies in testgrid.json file if present */
  if(utils.isUndefined(packageVersion) && bsConfig && (process.env.TESTGRID_AUTOMATION == "true" || process.env.TESTGRID_AUTOMATION == "1")) {
    ...
    if (runSettings && runSettings.npm_dependencies !== undefined && 
      ...
      if (package_ in runSettings.npm_dependencies) {
        ...
        logger.info(`Getting ${package_} package version from testgrid.json = ${packages[package_]}`);
        packageVersion = packages[package_];
      }
    }
  }
...
const getCypressCommandEventListener = () => {
  return (
    `require('testgrid-cypress-cli/bin/testObservability/cypress');`
  );
}
...
exports.setCrashReportingConfigFromReporter = (credentialsStr, bsConfigPath, cypressConfigPath) => {
  try {
    const testgridConfigFile = utils.readBsConfigJSON(bsConfigPath);
    ...
    CrashReporter.getInstance().setConfigDetails(credentialsStr, testgridConfigFile, cypressConfigFile);
  } catch(e) {
  ...
  if(BSTestOpsToken === '') {
    ...
    try {
      ...
      const data = {
        ...
        'build_run_identifier': process.env.TESTGRID_BUILD_RUN_IDENTIFIER,
        'failed_tests_rerun': process.env.TESTGRID_RERUN || false,
      ...
      // setEventListeners();
      if(this.isTestgridInfra()) helper.setTestgridCypressCliDependency(user_config);
    } catch(error) {
      if(!error.errorType) {
        if (error.response) {
...
exports.setTestObservabilityFlags = (bsConfig) => {
  ...
  try {
    ...
    if(!utils.isUndefined(process.env.TESTGRID_TEST_OBSERVABILITY)) isTestObservabilitySession = ( process.env.TESTGRID_TEST_OBSERVABILITY == "true" || process.env.TESTGRID_TEST_OBSERVABILITY == "1" );
  ...
  /* testgridAutomation */
  let isTestgridInfra = true;
  try {
    if(!utils.isUndefined(bsConfig["testgridAutomation"])) isTestgridInfra = ( bsConfig["testgridAutomation"] == true || bsConfig["testgridAutomation"] == 1 );
    if(!utils.isUndefined(process.env.TESTGRID_AUTOMATION)) isTestgridInfra = ( process.env.TESTGRID_AUTOMATION == "true" || process.env.TESTGRID_AUTOMATION == "1" );
    if(process.argv.includes('--disable-testgrid-automation')) isTestgridInfra = false;
  } catch(e) {
    isTestgridInfra = true;
  ...
  return [isTestObservabilitySession, isTestgridInfra];
}

exports.isTestObservabilitySession = () => {
  return ( process.env.TESTGRID_TEST_OBSERVABILITY == "true" );
}

exports.isTestgridInfra = () => {
  return ( process.env.TESTGRID_AUTOMATION == "true" );
}

exports.shouldReRunObservabilityTests = () => {
  return (process.env.TESTGRID_RERUN_TESTS && process.env.TESTGRID_RERUN_TESTS !== "null") ? true : false
}
...
const getReRunSpecs = (rawArgs) => {
  if (this.isTestObservabilitySession() && this.shouldReRunObservabilityTests()) {
    ...
    return [...rawArgs, '--spec', process.env.TESTGRID_RERUN_TESTS];
  } else {
    return rawArgs;
  }
}
