...
exports.requireModule = (module, internal = false) => {
  let local_path = "";
  if(process.env["testGridCwd"]){
   local_path = path.join(process.env["testGridCwd"], 'node_modules', module);
  } else if(internal) {
    local_path = path.join(process.cwd(), 'node_modules', 'testgrid-cypress-cli', 'node_modules', module);
  } else {
...
class CrashReporter {
  ...
  setConfigDetails(credentialsStr, testgridConfigFile, cypressConfigFile) {
    /* User test config for build run */
    this.userConfigForReporting = {
      framework: 'Cypress',
      testgridConfigFile: testgridConfigFile,
    ...
    try {
      ...
      const data = {
          ...
          observability_version: {
              ...
              frameworkVersion: getPackageVersion('cypress', this.userConfigForReporting.testgridConfigFile),
      ...
      request(options, function callback(error, response, body) {
        if(error) {
          ...
          debug(`[Crash_Report_Upload] Failed due to ${response && response.body ? response.body : `Received response from TestGrid Server with status : ${response.statusCode}`}`);
        } else {
