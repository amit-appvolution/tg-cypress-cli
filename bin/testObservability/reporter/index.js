...
class MyReporter {
  ...
  sendTestRunEvent = async (test, err = undefined, customFinished=false, eventType = "TestRunFinished") => {
    try {
      if(test.body && test.body.match(/testgrid internal helper hook/)) return;
      let failureArgs = [];
      ...
      if(!isTestgridInfra()) gitConfigPath = process.env.OBSERVABILITY_GIT_CONFIG_PATH_LOCAL ? process.env.OBSERVABILITY_GIT_CONFIG_PATH_LOCAL.toString() : null;
      ...
      let testData = {
        ...
        'vc_filepath': !isTestgridInfra() ? ( gitConfigPath ? path.relative(gitConfigPath, rootParentFile) : null ) : ( gitConfigPath ? ((gitConfigPath == 'DEFAULT' ? '' : gitConfigPath) + fileSeparator + rootParentFile).replaceAll("\\", "/") : null ),
