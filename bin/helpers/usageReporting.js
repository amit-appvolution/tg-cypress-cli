...
function bstack_json_found_in_pwd() {
  try {
    if (fs.existsSync(path.join(process.cwd(), 'testgrid.json'))) {
      //file exists
      return true;
    }
    else {
...
function cli_version_and_path(bsConfig) {
  ...
  if (bsConfig && bsConfig.run_settings && bsConfig.run_settings.cypressProjectDir) {
    let _path = path.join(bsConfig.run_settings.cypressProjectDir, 'node_modules', 'testgrid-cypress');
    let version = get_version(_path);
    if (!version) {
      version = get_version('testgrid-cypress');
    ...
    let version = get_version('testgrid-cypress');
