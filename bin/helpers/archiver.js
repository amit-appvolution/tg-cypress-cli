...
const archiveSpecs = (runSettings, filePath, excludeFiles, md5data) => {
  return new Promise(function (resolve, reject) {
    if (md5data.zipUrlPresent) {
      logger.debug("Skipping test suite upload since TestGrid already has your test suite that has not changed since the last run.");
    ...
    if (Object.keys(packageJSON).length > 0) {
      ...
      archive.append(packageJSONString, {name: `${cypressAppendFilesZipLocation}testgrid-package.json`});
    }
