...
exports.getBuildDetails = (bsConfig, isO11y = false) => {
  ...
  buildName = process.env.TESTGRID_BUILD_NAME || buildName;
  projectName = process.env.TESTGRID_PROJECT_NAME || projectName;
...
exports.setTestgridCypressCliDependency = (bsConfig) => {
  const runSettings = bsConfig.run_settings;
  if (runSettings.npm_dependencies !== undefined && 
    typeof runSettings.npm_dependencies === 'object') {
    if (!("testgrid-cypress-cli" in runSettings.npm_dependencies)) {
      logger.warn("Missing testgrid-cypress-cli not found in npm_dependencies");        
      runSettings.npm_dependencies['testgrid-cypress-cli'] = this.getAgentVersion() || "latest";
      logger.warn(`Adding testgrid-cypress-cli version ${runSettings.npm_dependencies['testgrid-cypress-cli']} in npm_dependencies`);
    }
  }
}
