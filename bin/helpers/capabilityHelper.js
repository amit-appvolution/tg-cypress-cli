...
const validate = (bsConfig, args) => {
  return new Promise(function (resolve, reject) {
    ...
    if (!bsConfig) reject(Constants.validationMessages.EMPTY_TESTGRID_JSON);

    if (!bsConfig.auth) reject(Constants.validationMessages.INCORRECT_AUTH_PARAMS);

    if( bsConfig.auth.username == "<Your TestGrid username>" || bsConfig.auth.access_key == "<Your TestGrid access key>" ) reject(Constants.validationMessages.INVALID_DEFAULT_AUTH_PARAMS);
    ...
    if ( bsConfig && bsConfig.run_settings && bsConfig.run_settings.enforce_settings && bsConfig.run_settings.enforce_settings.toString() === 'true' && Utils.isUndefined(bsConfig.run_settings.specs) ) {
      reject(Constants.validationMessages.EMPTY_SPECS_IN_TESTGRID_JSON);
    }

    // validate parallels specified in testgrid.json if parallels are not specified via arguments
    ...
    try {
      ...
      if (Utils.isUndefinedOrFalse(bsConfig.run_settings.enforce_settings) && bsConfig.run_settings.cypress_config_filename !== 'false') {
        ...
        logger.debug("Validating baseurl and integrationFolder in testgrid.json");
