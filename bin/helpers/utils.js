    ...
    try {
      ...
      reject(
        e.code === "MODULE_NOT_FOUND"
          ? "Couldn't find the testgrid.json file at \"" +
              bsConfigPath +
              '". Please use --config-file <path to testgrid.json>.'
          : `Invalid testgrid.json file. Error : ${e.message}`
      );
    }
  });
};
...
exports.getErrorCodeFromMsg = (errMsg) => {
  let errorCode = null;
  switch (errMsg) {
    case Constants.validationMessages.EMPTY_TESTGRID_JSON:
  ...
  if (
    errMsg.includes("Please use --config-file <path to testgrid.json>.")
  ) {
    errorCode = "bstack_json_path_invalid";
  } else if (errMsg.includes("Invalid testgrid.json file.")) {
...
exports.setDefaults = (bsConfig, args) => {
  ...
  if (this.isUndefined(bsConfig['auth']) && (!this.isUndefined(args.username) || !this.isUndefined(process.env.TESTGRID_USERNAME))) {
    bsConfig['auth'] = {};
  }
...
exports.setUsername = (bsConfig, args) => {
  ...
  } else if (!this.isUndefined(process.env.TESTGRID_USERNAME)) {
    bsConfig["auth"]["username"] = process.env.TESTGRID_USERNAME;
    logger.info(
      "Reading username from the environment variable TESTGRID_USERNAME"
    );
  }
};
...
exports.setAccessKey = (bsConfig, args) => {
  ...
  } else if (!this.isUndefined(process.env.TESTGRID_ACCESS_KEY)) {
    bsConfig["auth"]["access_key"] = process.env.TESTGRID_ACCESS_KEY;
    logger.info(
      "Reading access key from the environment variable TESTGRID_ACCESS_KEY"
    );
  }
};
...
exports.setUserSpecs = (bsConfig, args) => {
  if(o11yHelpers.isTestgridInfra() && o11yHelpers.isTestObservabilitySession() && o11yHelpers.shouldReRunObservabilityTests()) {
    bsConfig.run_settings.specs = process.env.TESTGRID_RERUN_TESTS;
    return;
  }
...
exports.setSystemEnvs = (bsConfig) => {
  ...
  try {
    ...
    if(!o11yHelpers.isTestgridInfra()) process.env.OBSERVABILITY_GIT_CONFIG_PATH_LOCAL = gitConfigPath;
    if(gitConfigPath) {
...
exports.exportResults = (buildId, buildUrl) => {
  ...
  fs.writeFileSync("log/build_results.txt", data, function (err) {
    if (err) {
      logger.warn(
        `Couldn't write BUILD_ID with value: ${buildId} to testgrid/build_results.txt`
      );
      logger.warn(
        `Couldn't write BUILD_URL with value: ${buildUrl} to testgrid/build_results.txt`
      );
    }
  });
};
...
exports.setLocal = (bsConfig, args) => {
  ...
  } else if (!this.isUndefined(process.env.TESTGRID_LOCAL)) {
    let local = false;
    if (String(process.env.TESTGRID_LOCAL).toLowerCase() === 'true') {
      local = true;
    }
    bsConfig['connection_settings']['local'] = local;
    logger.info(
      'Reading local setting from the environment variable TESTGRID_LOCAL'
    );
...
exports.setLocalIdentifier = (bsConfig, args) => {
  ...
  } else if (!this.isUndefined(process.env.TESTGRID_LOCAL_IDENTIFIER)) {
    bsConfig["connection_settings"]["local_identifier"] =
      process.env.TESTGRID_LOCAL_IDENTIFIER;
    logger.info(
      "Reading local identifier from the environment variable TESTGRID_LOCAL_IDENTIFIER"
    );
    bsConfig['connection_settings']['local_mode'] = 'always-on';
  } else if (
...
exports.deleteBaseUrlFromError = (err) => {
  return err.replace(/To test ([\s\S]*)on TestGrid/g, 'To test on TestGrid');
}
...
exports.stopTestGridBuild = async (bsConfig, args, buildId, rawArgs, buildReportData = null) => {
  let that = this;
  return new Promise(function (resolve, reject) {
...
async function processExitHandler(exitData){
  ...
  await this.stopTestGridBuild(exitData.bsConfig, exitData.args, exitData.buildId, null, exitData.buildReportData);
