...
const syncCLI = {
  ...
  DEFAULT_LINE_SEP:
    "\n--------------------------------------------------------------------------------",
  STARTUP_MESSAGE:
    "TestGrid machines are now setting up Cypress with the specified npm dependencies for running your tests. It might take some time before your tests start runnning and showing up below...",
};
...
const userMessages = {
  ...
  CONFIG_FILE_EXISTS:
    "File already exists, delete the testgrid.json file manually. skipping...",
  DIR_NOT_FOUND:
    "Given path does not exist. Failed to create testgrid.json in %s",
  MD5_CHECK_FAILED:
  ...
  NPM_UPLOAD:
    "Building the package to upload to TestGrid",
  ...
  FAILED_MD5_CHECK:
    "Something went wrong - you can retry running testgrid-cypress with ‘--force-upload’ parameter, or contact TestGrid Support.",
  ...
  UPLOADING_NPM_PACKAGES: "Uploading required node_modules to TestGrid",
  UPLOADING_NPM_PACKAGES_SUCCESS: "Uploaded node_modules successfully",
  SKIP_UPLOADING_TESTS:
    "Skipping zip upload since TestGrid already has your test suite that has not changed since the last run.",
  SKIP_NPM_INSTALL:
    "Skipping NPM Install as the enforce_settings has been passed.",
  SKIP_UPLOADING_NPM_PACKAGES:
    "Skipping the upload of node_modules since TestGrid has already cached your npm dependencies that have not changed since the last run.",
  ...
  ASYNC_DOWNLOADS:
    "Test artifacts as specified under 'downloads' can be downloaded after the build has completed its run, using 'testgrid-cypress generate-downloads <build-id>'",
...
const validationMessages = {
  ...
  NOT_VALID: "testgrid.json is not valid",
...
const cliMessages = {
  ...
  INIT: {
    INFO: "create a testgrid.json file in the folder specified with the default configuration options.",
  ...
  RUN: {
    ...
    INFO: "Run your tests on TestGrid. For more help: `testgrid-cypress run --help`.",
    ...
    NODE_VERSION: "Pass the node version that you want TestGrid to use to run your Cypress tests on.",
    BUILD_TAG: "Add a tag to your build to filter builds based on tag values on the Dashboard."
  },
  COMMON: {
    DISABLE_USAGE_REPORTING: "Disable usage reporting",
    FORCE_UPLOAD:
      "Force the upload of your test files even if TestGrid has detected no changes in your suite since you last ran",
    USERNAME: "Your TestGrid username",
    ACCESS_KEY: "Your TestGrid access key",
    NO_NPM_WARNING: "No NPM warning if npm_dependencies is empty",
    CONFIG_DEMAND: "config file is required",
    CONFIG_FILE_PATH: "Path to TestGrid config",
