...
module.exports = function run(args, rawArgs) {
  ...
  logger.debug(`testgrid.json path : ${bsConfigPath}`);
  ...
  logger.debug('Started testgrid.json validation');
  return utils.validateBstackJson(bsConfigPath).then(async function (bsConfig) {
    markBlockEnd('validateBstackJson');
    logger.debug('Completed testgrid.json validation');
    markBlockStart('setConfig');
    logger.debug('Started setting the configs');

    /* 
      Set testObservability & testgridAutomation flags
    */
    const [isTestObservabilitySession, isTestgridInfra] = setTestObservabilityFlags(bsConfig);
    ...
    let buildReportData = (turboScaleSession || !isTestgridInfra) ? null : await getInitialDetails(bsConfig, args, rawArgs);
    ...
    */
    ...
    if(isTestgridInfra) {
      ...
      if (isAccessibilitySession && isTestgridInfra) {
      ...
      if (turboScaleSession) {
        ...
        // Even then, it will be spawned separately via testgrid-cli ats connect-grid command and not via testgrid-cypress-cli
    ...
    const { packagesInstalled } = !isTestgridInfra ? false : await packageInstaller.packageSetupAndInstaller(bsConfig, config.packageDirName, {markBlockStart, markBlockEnd});
    ...
    // Validate testgrid.json values and parallels specified via arguments
    ...
    return capabilityHelper.validate(bsConfig, args).then(function (cypressConfigFile) {
      ...
      markBlockStart('zip');
      logger.debug("Checking if test suite zip and dependencies is already available on testgrid");
        ...
        return packageInstaller.packageWrapper(bsConfig, config.packageDirName, config.packageFileName, md5data, {markBlockStart, markBlockEnd}, packagesInstalled).then(function (packageData) {
          ...
          return archiver.archive(bsConfig.run_settings, config.fileName, args.exclude, md5data).then(async function (data) {
            ...
            return zipUploader.zipUpload(bsConfig, md5data, packageData).then(async function (zip) {
              ...
              if (process.env.TESTGRID_TEST_ACCESSIBILITY === 'true') {
                supportFileCleanup();
              }
              ...
              logger.debug("Started setting up TestGrid Local connection");
              let bs_local = await utils.setupLocalTesting(bsConfig, args, rawArgs, buildReportData);
              logger.debug('Completed setting up TestGrid Local connection');
              ...
              return build.createBuild(bsConfig, zip).then(function (data) {
                ...
                if (args.sync) {
                  logger.debug("Started polling build status from TestGrid");
                ...
                if (err && err.toString().includes('testgrid.geoLocation')) {
                  err = err.replace(/testgrid.geoLocation/g, 'geolocation');
                }
      ...
      // display testgrid.json is not valid only if validation of testgrid.json field has failed, otherwise display just the error message
      // If parallels specified in arguments are invalid do not display testgrid.json is invalid message
