...
function get_path(args, rawArgs) {
  if (args._.length > 1 && args.p) {
    ...
    return path.join(args.p, "testgrid.json");
  ...
  return path.join(process.cwd(), "testgrid.json");
}
...
module.exports = function init(args, rawArgs) {
  ...
  // append testgrid.json if filename is a path without filename
  if (path.extname(path_to_json) === '') path_to_json = path.join(path_to_json, "testgrid.json");
